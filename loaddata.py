...
def processName():
    ...
    if keep_scaled:
        scaler = preprocessing.StandardScaler()
        df['Names_scaled'] = scaler.fit_transform(df['Names'].values.reshape(-1, 1) )
...
def processAge():
    ...
    if keep_bins and keep_scaled:
        scaler = preprocessing.StandardScaler()
        df['Age_bin_id_scaled'] = scaler.fit_transform(df['Age_bin_id'].values.reshape(-1, 1) )
...
if __name__ == '__main__':
    ...
    # train.drop(['PassengerId'], axis=1, inplace=1)
    # test.drop(['PassengerId'], axis=1, inplace=1)
